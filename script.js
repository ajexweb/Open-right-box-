// Initialize state from localStorage to ensure persistence across sessions
let state = {
...
const elements = {
    ...
    resetGlobalData: document.getElementById('reset-global-data'),
...
// Save state to localStorage to ensure global persistence
...
// Display users by date with delete buttons
        ...
        <tbody>
            ${filteredUsers.map(u => `
                <tr>
                    ...
                    <td><button class="delete-user-btn" data-username="${u.username}">Delete</button></td>
                </tr>
            `).join('')}
        </tbody>
    `;
...
function deleteUser(username) {
    ...
    // Update references in invitedBy and invitedUsers
    state.users.forEach(u => {
        if (u.invitedBy === username) {
            u.invitedBy = null;
        }
        if (u.invitedUsers) {
            u.invitedUsers = u.invitedUsers.filter(invited => invited.username !== username);
        }
    });
    ...
    // If the deleted user was logged in, redirect to login
    if (!state.loggedInUser) {
        showSection('loginSection');
    }
}
...
// Reset global data
function handleResetGlobalData() {
    if (confirm('Are you sure you want to reset all global data? This will delete all users and ads!')) {
        state.ads = [];
        state.users = [];
        state.loggedInUser = null;
        saveState();
        ...
        displayAds();
        showSection('loginSection');
        alert('Global data has been reset.');
    }
}
...
function setupEventListeners() {
    ...
    const buttons = [
        ...
        'uploadAdBtn', 'adminLogout', 'resetGlobalData', 'profileBtn', 'editProfileBtn', 'saveProfileBtn',
    ...
    if (elements.resetGlobalData) elements.resetGlobalData.addEventListener('click', handleResetGlobalData);
